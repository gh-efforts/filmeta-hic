...
type options struct {
	...
	path                    string
	prefix                  bool
	enableFormatDotNotation bool
}

func WithEnableFormatDotNotation(ok bool) Option {
	return func(o *options) {
		o.enableFormatDotNotation = ok
	}
}
...
func (s *source) Load() ([]*config.KeyValue, error) {
	...
	if s.options.ctx == nil {
		...
		s.options.ctx, cancelFn = context.WithTimeout(context.Background(), 10*time.Second)
		defer cancelFn()
	}
	...
	for _, item := range rsp.Kvs {
		k := strings.TrimPrefix(string(item.Key), s.options.path)
		if s.options.enableFormatDotNotation {
			k = toReplacePath(k)
		}
...
func toReplacePath(path string) string {
