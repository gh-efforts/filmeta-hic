...
func TestBufferNegative(t *testing.T) {
	var result int
	_, err := Just(1, 2, 3, 4).Buffer(-1).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
		for item := range pipe {
			result += item.(int)
		}
		return result, nil
...
func TestJust(t *testing.T) {
	var result int
	_, err := Just(1, 2, 3, 4).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
		for item := range pipe {
			result += item.(int)
		}
		return result, nil
...
func TestFilter(t *testing.T) {
	var result int
	_, err := Just(1, 2, 3, 4).Filter(func(item interface{}) bool {
		return item.(int)%2 == 0
	}).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
		for item := range pipe {
			result += item.(int)
...
func TestHead(t *testing.T) {
	var result int
	_, err := Just(1, 2, 3, 4).Head(2).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
		for item := range pipe {
			result += item.(int)
		}
		return result, nil
...
func TestHeadZero(t *testing.T) {
	assert.Panics(t, func() {
		_, err := Just(1, 2, 3, 4).Head(0).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
			return nil, nil
		})
		if err != nil {
			return
...
func TestHeadMore(t *testing.T) {
	var result int
	_, err := Just(1, 2, 3, 4).Head(6).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
		for item := range pipe {
			result += item.(int)
		}
		return result, nil
...
func TestTail(t *testing.T) {
	var result int
	_, err := Just(1, 2, 3, 4).Tail(2).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
		for item := range pipe {
			result += item.(int)
		}
		return result, nil
...
func TestTailZero(t *testing.T) {
	assert.Panics(t, func() {
		_, err := Just(1, 2, 3, 4).Tail(0).Reduce(func(pipe <-chan interface{}) (interface{}, error) {
			return nil, nil
		})
		if err != nil {
			return
...
func BenchmarkMapReduce(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := From(func(input chan<- interface{}) {
			for j := 0; j < 2; j++ {
				input <- int64(j)
			}
